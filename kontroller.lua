...
if( next_gun or previous_gun ) then
	...
	GamePlaySound( "mods/kappa/sfx/kappa.bank", "gun_switch", x, y )
end
...
if( ModSettingGetNextValue( "kappa.SHOW_POINTER" )) then
	...
	local pic = "mods/kappa/pointer/"..math.max( kappa_id, 1 ).."pointer"
...
if( ModSettingGetNextValue( "kappa.SHOW_UI" )) then
	...
	uid = pen.new_image( gui, uid, pic_x - length/2, pic_y, pic_z - 0.5, "mods/kappa/pixels_white.png", length, bar_height )
	...
	if( wand_in_hand > 0 ) then
		...
		if( full_frame > 1 ) then
			uid = pen.new_image( gui, uid, pic_x - full_frame/2, pic_y - ( is_pinned and 2 or 0 ), pic_z - 0.55, "mods/kappa/pixels_blue.png", full_frame, is_pinned and 1.5 or 2 )
		end
	end
end
